...
| 檔案 | 內容 |
|---|---|
| `init-project-flow.md` | `npx dflow init` 的 internal flow / manual fallback，定義 project bootstrap 問答和 scaffolding 行為 |
...
| `CLAUDE.md` | init 用的 AI 協作規範模板來源；採用時由 `npx dflow init` 處理 |
...
1. 在練習專案根目錄執行 `npx dflow init`，選擇 Core edition
2. 啟動 Claude Code，輸入：
```
