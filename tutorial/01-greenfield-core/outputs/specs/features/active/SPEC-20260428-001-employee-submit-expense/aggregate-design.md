...
| `Reject(approverId, reason, decidedAt)` <!-- phase-2 ADDED --> | Status == Submitted；approverId != SubmitterId；reason 至少 10 字元 | Status = Rejected；RejectedAt = decidedAt | **ExpenseReportRejected** (ReportId, SubmitterId, ApproverId, RejectedAt, Reason, SubmitAttemptNo) |
...
| INV-09 <!-- phase-2 ADDED --> | Rejected decision 必須有 ApprovalReason，且至少 10 字元 | `ApprovalReason` 建構式拋 `DomainException("Reject reason must contain at least 10 characters.")` |
...
| `CreateRejected(reportId, submitterId, submitAttemptNo, approverId, reason, decidedAt)` | `approverId != submitterId`；reason 至少 10 字元；尚無同 `(reportId, submitAttemptNo)` decision | 回傳 Decision = Rejected 的 ApprovalDecision | Event 由 ExpenseReport.Reject() raise |
