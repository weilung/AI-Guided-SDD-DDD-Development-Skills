...
**Behavior before**: Given ExpenseReport.Status = Submitted / When AddItem, RemoveItem, ModifyItem / Then raise DomainException("Cannot modify a submitted ExpenseReport.") / no event.

**Behavior after**: Given ExpenseReport.Status = Submitted / When AddItem, RemoveItem, ModifyItem / Then raise DomainException("Cannot modify a submitted ExpenseReport.") / no event. Given ExpenseReport.Status = Rejected / When AddItem, RemoveItem, ModifyItem / Then edit is allowed and Status returns to Draft on first edit, so the employee can Submit again and create a new ApprovalDecision for the new submit attempt.

**Reason**: phase 2 引入主管退回。Rejected 若仍完全不可編輯，員工只能重新建一張單，會切斷原 Report 的 audit trail；允許 Rejected 重編能保留同一 Report 的歷史，同時讓每次 Submit 對應新的 ApprovalDecision。
...
- Rejected 後重新 Submit 是否要保留完整 item edit history？phase 2 只保留 ApprovalDecision 與 Report 狀態歷史；item-level audit trail 留到審計需求明確後再評。
- Approval policy 若未來出現金額門檻、二階主管、代理人等複雜規則，需重新 review 是否拆成獨立 Approval BC。
