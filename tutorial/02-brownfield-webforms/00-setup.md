...
| 主要 entry point | `/dflow:init-project` 後接 `/dflow:new-feature`，從第一個 feature 建立 Domain。 | 本段先提供 init baseline，後續直接進 `/dflow:modify-existing`，從既有 Code-Behind 的修改需求切入。 |
...
Bob 不會在劇情 2 一次處理所有候選 Bounded Context。段 2 的合理策略是先選一個具體、範圍不大的 Order 修改，讀既有行為、捕捉 baseline，再決定是否建立第一個 `specs/domain/{context}/`。
...
## 段 1 init-project 暫緩說明

劇情 2 的 `01-init-project.md` 暫緩，原因是 init-project 是否升級為 CLI 還在待決議中（per `planning/public-distribution-backlog.md` § 2.1）。因此本段 0 不扮演 Bob 與 Dflow 的對話，也不觸發 `/dflow:` 命令；它直接提供一組「假設 Bob 已跑完 `/dflow:init-project`」的 baseline outputs，讓後續 `02-modify-existing.md` 可以從已初始化的 brownfield 專案開始。

議題拍板後，劇情 2 可以回頭補 `01-init-project.md`，把 Bob 實際跑 init 的互動流程補齊；本段建立的 baseline 會作為那段的目標結果。

## baseline outputs 在哪裡

baseline outputs 位於 [`tutorial/02-brownfield-webforms/outputs/`](outputs/)。這些檔案模擬 Bob 在 OrderManager 裡完成 `/dflow:init-project` 後的結果。
...
| Q5 optional | `_overview.md`、`_conventions.md`、`Git-principles-gitflow.md`、`CLAUDE.md` snippet 全選 |

baseline 檔案清單：

| Path | 用途 |
|---|---|
| `outputs/specs/features/active/.gitkeep` | 保留 active feature 工作區，目前沒有任何 feature 目錄。 |
| `outputs/specs/features/completed/.gitkeep` | 保留 completed feature 歸檔區。 |
| `outputs/specs/features/backlog/.gitkeep` | 保留 backlog feature 工作區。 |
| `outputs/specs/domain/glossary.md` | OrderManager 的 Ubiquitous Language 起點，先放核心術語與 open questions。 |
| `outputs/specs/domain/context-map.md` | 預期 Bounded Context 候選清單：Order / Customer / Inventory / Shipment / Invoice，並標註邊界待第一個 modify 確認。 |
| `outputs/specs/architecture/tech-debt.md` | baseline tech debt backlog，記錄 Code-Behind、測試、Stored Procedures、migration 與 DI 缺口。 |
| `outputs/specs/shared/_overview.md` | 系統 overview、current architecture、migration context note 與 known pain points。 |
| `outputs/specs/shared/_conventions.md` | spec 撰寫慣例，含 brownfield modify-existing 的專案級補充。 |
| `outputs/specs/shared/Git-principles-gitflow.md` | Bob 選擇 Git Flow 的 Git 規範。 |
| `outputs/CLAUDE.md` | repo root AI 協作規則，含 OrderManager 專案描述、Domain Layer Rules 與 AI collaboration notes。 |

本 baseline 不建立任何 `specs/domain/{BC}/` 子目錄，也不建立任何 `specs/features/active/SPEC-...` 目錄。第一個 Bounded Context 與第一個 active spec 會由後續 modify-existing 或 new-feature flow 依實際需求建立。

## 下一個劇情段

→ `02-modify-existing.md`：Bob 用 modify-existing flow 處理第一個 brownfield modification。

本劇情刻意跳過 `01-init-project.md`，原因見上方「段 1 init-project 暫緩說明」。
