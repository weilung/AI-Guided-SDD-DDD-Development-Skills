...
你是 P001（Workflow Transparency / Hybrid 指令化入口）和 P007b（PR Review Spec Intent Step 0）實作的審查者。本次 review 範圍限定這兩份 proposal 的實作產物，不含其他 proposal。
...
**實作產物（繁中版，人閱讀的檔案）**
12. `sdd-ddd-webforms-skill-tw/SKILL_tw.md`
13. `sdd-ddd-core-skill-tw/SKILL_tw.md`
14. `sdd-ddd-webforms-skill-tw/new-feature-flow_tw.md`
15. `sdd-ddd-core-skill-tw/new-feature-flow_tw.md`
16. `sdd-ddd-webforms-skill-tw/modify-existing-flow_tw.md`
17. `sdd-ddd-core-skill-tw/modify-existing-flow_tw.md`
18. `sdd-ddd-webforms-skill-tw/pr-review-checklist_tw.md`
19. `sdd-ddd-core-skill-tw/pr-review-checklist_tw.md`

**流程規範（本次 review 格式要求）**
20. `proposals/review-workflow.md`（第三節「Review 報告格式」定義輸出結構）

## 第二步：審查重點

照以下五個角度各看一輪。不是 checklist——是 lens：
...
4. **英文版 vs 繁中版同步**：段落結構、範例內容、指令清單應該完全對應。用字風格可以不同（英文精確、繁中口語），但資訊量不能落差。
5. **範圍邊界**：實作有沒有夾帶超出 P001/P007b 範圍的改動？若有發現可能是其他 proposal（P002 Delta、P003 behavior.md 等）相關的改動，記下但不在本輪深查。

## 第三步：關注的具體特徵（供參考，不限於此）
...
- 兩版一致性對照表和繁中同步對照表都必須有（即使全 ✓ 也要列，證明你檢查過）
- Findings 要有明確證據（引用 line range），不是印象描述

## 禁止事項

- 不要修改任何實作檔。本輪只產出 review 報告。
- 不要評論 P001/P007b 以外的 proposal 實作（若發現牽涉到 P002/P003 等，在報告末尾另起一段「跨輪發現」，不計入本輪 Finding）。
...
若你看到的問題明顯不屬於 P001/P007b 範圍、但屬於另一輪將審查的範圍（例如 R2 的 Delta 格式、R4 的 behavior.md），請寫在 `reviews/cross-round-notes.md`（若檔案不存在則建立）。格式：
...
報告完成後告訴使用者：「Round 1 review 報告已產出於 reviews/round-1-report.md，共 N 個 findings（Critical: X / Major: Y / Minor: Z / Question: W）。可進入 Approve 階段（另開新 Claude Code session）。」
