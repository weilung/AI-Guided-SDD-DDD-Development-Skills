...
1. `proposals/review-workflow.md`（本流程的規範——特別是第一節角色分工、第三節 Approval 決議格式）
...
5. `reviews/cross-round-notes.md`（若存在）——僅供參考，本輪不處理跨輪項
...
## 第三步：處理方式（一次一項，不 dump）

對報告的每個 Finding：

1. 重述 Finding 的核心（不超過兩句）
2. 說出你的**建議決議**（accept / reject / defer / clarify）
...
5. 等使用者確認你的建議（accept / reject / defer / clarify）或給不同決議，確認後再處理下一項
...
## 第五步：全部 Finding 處理完後，產出 decisions 文件
...
> - 共 N 個 findings 處理完畢（accept: X / reject: Y / defer: Z / clarify→XX: W）
