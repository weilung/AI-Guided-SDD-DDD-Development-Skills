...
目前正式 flow 仍使用 `specs/_共用/`。P012 若通過，machine-facing 路徑會改為 `specs/shared/`；下方先用 `shared/` 表示目標方向，並在註解標明目前名稱。
...
├── specs/
│   ├── shared/                                   # 目前 flow 寫作 specs/_共用/；P012 後預期改名
...
- `specs/_共用/` 是 machine-facing path，不利於公開散佈與跨工具使用。
