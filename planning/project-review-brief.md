...
R9 / P014 後，正式 init baseline 使用 `npx dflow-sdd-ddd init`，Dflow-managed 文件位於 `dflow/specs/`。`CLAUDE.md` 是 root-level 特例，保留在專案根目錄供 AI tools 探測。
...
├── dflow/
│   └── specs/
│       ├── shared/                               # 專案級治理文件（init 階段建立 / 選擇性寫入）
│       │   ├── _overview.md                      # optional scaffolding：系統概覽、遷移策略
│       │   ├── _conventions.md                   # mandatory：文件與協作慣例，含 Prose Language
...
│       │   └── backlog/
│       ├── domain/
│       │   ├── glossary.md                       # mandatory baseline
...
| Path | 來源 / 性質 |
|---|---|
| `dflow/specs/features/active/.gitkeep` | 空目錄佔位 |
| `dflow/specs/features/completed/.gitkeep` | 空目錄佔位 |
| `dflow/specs/features/backlog/.gitkeep` | 空目錄佔位 |
| `dflow/specs/shared/_conventions.md` | mandatory baseline；含 `## Prose Language` |
...
| Path | 理由 |
|---|---|
| `dflow/specs/domain/{context}/behavior.md` | 由 `/dflow:new-feature` 完成時或 brownfield baseline capture 建立，避免空 placeholder 漂移 |
| `dflow/specs/domain/context-map.md` | WebForms 版讓 Bounded Context 從既有系統抽離過程自然浮現 |
...
│       ├── shared/                               # 專案級治理文件（init 階段建立 / 選擇性寫入）
│       │   ├── _overview.md
│       │   ├── _conventions.md                   # mandatory：含 Prose Language
...
| `dflow/specs/shared/_conventions.md` | mandatory baseline；含 `## Prose Language` |
| `dflow/specs/domain/glossary.md` | mandatory baseline |
| `dflow/specs/domain/context-map.md` | mandatory baseline |
| `dflow/specs/architecture/tech-debt.md` | mandatory baseline |
| `dflow/specs/architecture/decisions/README.md` | mandatory baseline，作為 ADR 目錄說明 |

Core init 不會先建立：

| Path | 理由 |
|---|---|
| `dflow/specs/domain/{context}/behavior.md` | 由 `/dflow:new-feature` 完成時或 brownfield baseline capture 建立 |
| `dflow/specs/architecture/decisions/ADR-*.md` | 架構決策出現時才建立，不在 Day 0 預先產生 |
...
| `scaffolding/` | `npx dflow-sdd-ddd init` 一次性種到專案中的輔助文件 | 專案初始化時使用，寫入後由目標專案維護 |
...
- 專案 bootstrap：npm CLI，V1 預設 `npx dflow-sdd-ddd init`。
- 日常 workflow 主要入口：明確的 `/dflow:` slash commands。
- 次要入口：自然語言 auto-trigger safety net。
- safety net 的意思不是自動進流程，而是 AI 判斷意圖後提示建議指令，等待開發者確認。

| 入口 | 用途 | 主要產物 |
|---|---|---|
| `npx dflow-sdd-ddd init` | 初始化專案，建立 `dflow/specs/` baseline 與 optional scaffolding | shared docs、domain / features / architecture 基礎目錄、可選 onboarding / Git / CLAUDE snippet |
...
| 檔案 | WebForms | Core | 作用 |
|---|---:|---:|---|
| `init-project-flow.md` | 有 | 有 | `npx dflow-sdd-ddd init` 的 internal flow spec / manual fallback 參考 |
...
- Git Flow 並沒有被完全刪除，而是移到 `npx dflow-sdd-ddd init` 可選 scaffolding：`scaffolding/Git-principles-gitflow.md`。Trunk / GitHub Flow 則對應 `scaffolding/Git-principles-trunk.md`。
...
### P012：distribution-friendly paths（已 implemented）

核心問題：

- `specs/_共用/` 是 machine-facing path，不利於公開散佈與跨工具使用。（已由 P012 處理為 `specs/shared/`，後續 P014 再將 runtime baseline 移到 `dflow/specs/`）
