...
R9 / P014 後，正式 init baseline 使用 `npx dflow init`，Dflow-managed 文件位於 `dflow/specs/`。`CLAUDE.md` 是 root-level 特例，保留在專案根目錄供 AI tools 探測。
...
| `scaffolding/` | `npx dflow init` 一次性種到專案中的輔助文件 | 專案初始化時使用，寫入後由目標專案維護 |
...
- 專案 bootstrap：npm CLI，V1 預設 `npx dflow init`。
...
| 入口 | 用途 | 主要產物 |
|---|---|---|
| `npx dflow init` | 初始化專案，建立 `dflow/specs/` baseline 與 optional scaffolding | shared docs、domain / features / architecture 基礎目錄、可選 onboarding / Git / CLAUDE snippet |
...
| 檔案 | WebForms | Core | 作用 |
|---|---:|---:|---|
| `init-project-flow.md` | 有 | 有 | `npx dflow init` 的 internal flow spec / manual fallback 參考 |
...
- Git Flow 並沒有被完全刪除，而是移到 `npx dflow init` 可選 scaffolding：`scaffolding/Git-principles-gitflow.md`。Trunk / GitHub Flow 則對應 `scaffolding/Git-principles-trunk.md`。
