...
- `npx dflow init` 必填 explicit project prose language。
...
- V1 預設入口：`npx dflow init`。
- `npm install -g dflow` + `dflow init` 只作為進階用法。
...
- Tutorial init 段已改為 shell 執行 `npx dflow init`。

仍待處理的是公開發布工程本身：package name availability、npm publish pipeline、versioning / release policy，以及多 AI 工具安裝說明。
...
目前已穩定 V1 machine-facing baseline：`npx dflow init` + `dflow/specs/` + canonical English structure + explicit project prose language。
