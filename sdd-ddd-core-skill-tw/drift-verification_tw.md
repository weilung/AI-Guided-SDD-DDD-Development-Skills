...
驗證 Core 版的 Context 時，額外檢查：
- `behavior.md` 中的 events.md 引用：如果情境寫到「And {DomainEvent} is raised」，確認該事件在 `events.md` 中有列出
- 這是**附加檢查**，不是阻擋性失敗——以警告方式回報：
  ```
  ⚠ BR-001 的情境引用了 ExpenseReportSubmitted 事件，
    但 events.md 中沒有列出
  ```
