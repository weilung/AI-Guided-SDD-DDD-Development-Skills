...
- **R1 審查修正（2026-04-20）**: F-01 — Core `SKILL.md` `/dflow:status` Response Format 範例中 Step 3 的名稱對齊到 `Domain Modeling`（status line 與 in-progress checklist 兩處），與 Core `new-feature-flow.md` 正式 heading 一致。詳見 `reviews/round-1-decisions.md` F-01 與 `CHANGELOG.md` 2026-04-20 (R1) 條目。
