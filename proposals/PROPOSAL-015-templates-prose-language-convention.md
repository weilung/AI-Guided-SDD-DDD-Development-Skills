...
1. `npx dflow init` 在 P014 的 CLI init flow 中新增 `prose-language` 問題。
...
4. 若 P014 延後或 init CLI 尚未落地，先由 `_conventions.md` scaffolding 提供相同段落，讓團隊手動填寫。

#### 候選做法比較

| 候選 | 機制說明 | 實作成本 | 維護成本 | AI 理解可靠度 | 與 P014 的耦合度 | 與 P013 anchor 機制的兼容 | 評估 |
|---|---|---:|---:|---|---|---|---|
| (A) `<!-- dflow:prose-language: any \| en \| xx-XX -->` directive | 在 templates / scaffolding 的檔頭放置 machine-readable directive；有 frontmatter 的 template 可放在 frontmatter 後、H1 前；無 frontmatter 的 scaffolding 可放在第一個 H1 前。生成後保留 directive，AI 看到該檔即可知道 prose 語言。 | 小到中 | 中 | 高，因為 directive 與目標檔同檔共址 | 低，可獨立於 CLI | 高。directive 與 `dflow:section` anchor 是不同用途，不互相衝突 | 適合需要 per-file override 的進階場景；但會在每份生成文件留下額外 metadata，且同一專案多檔 directive 可能 drift |
| (B) init CLI 新增 `prose-language` 問題 | `npx dflow init` 問使用者自由 prose 要用哪種語言，並寫入 `dflow/specs/shared/_conventions.md`。後續 AI workflow 讀 project convention。 | 中 | 低 | 高到中。初始化時明示，且集中在 project convention；可靠度取決於 workflow 是否固定讀 `_conventions.md` | 高。依賴 P014 Sub-wave 1 的 CLI init Q&A flow | 高。不改 heading / anchor，只規範 anchor 所定位段落內的 prose | **建議 default**。最符合公開散佈與團隊級 convention 心智模型，避免每份 template 重複標記 |
| (C) `_conventions.md` 新增 `Prose Language` 段 | 不新增 CLI 問題，只在雙版 `_conventions.md` scaffolding 加一段可手動填寫的 prose language policy；AI 依現有 convention 文件對齊。 | 小 | 低 | 中。只要 AI 有讀 `_conventions.md` 就穩定；若 workflow 沒提醒，可能被漏讀 | 低。可在 P014 之前或之後獨立落地 | 高。同 (B)，只管 prose，不碰 anchor / heading | **建議 fallback**。若 P014 尚未 approved / implemented，可先用此方案補洞 |
...
Default recommendation：**(B) init CLI `prose-language` 問題 + `_conventions.md` 持久化設定**。
...
Fallback recommendation：**(C) `_conventions.md` 手動段落**。

若 P014 尚未 approved、P014 implement 延後，或 CLI Q&A flow 尚未能穩定寫入 conventions，P015 可先落地 (C)：雙版 `_conventions.md` scaffolding 新增相同 `## Prose Language` 段，並在雙版 `SKILL.md` / workflow 指引中要求 AI 讀取。等 P014 CLI init 落地後，再把同一段由 CLI 問答自動填入。這個 fallback 不會與 (B) 衝突，因為 (B) 的持久化位置本來就是 `_conventions.md`。

#### Init CLI 對接點

P014 Sub-wave 1 建立 npm package skeleton 與 `npx dflow init` Q&A flow 後，P015 implement 應在 init 問題中新增：

```text
Project prose language for generated spec content?
Examples:
- any    = no strict preference; follow the user's current request and existing document language
- en     = write free prose in English
- zh-TW  = write free prose in Traditional Chinese
- custom = enter another BCP-47 language tag
```

建議儲存值：

```text
any | en | zh-TW | {BCP-47 language tag}
```

建議 default：

- public package default：`any`
...
> 若 P014 尚未 implement，現有路徑仍是 `specs/shared/_conventions.md`；但 P015 的目標設計應以 P014 後的 `dflow/specs/shared/_conventions.md` 為準。
...
- `any`: no strict preference. Follow the user's current request and the
  dominant prose language already present in the target document.
- `en`: write free prose in English.
- `{xx-XX}`: write free prose in that BCP-47 language, such as `zh-TW`.
...
因此 (A) 建議保留為未來 per-file override 的可能方向，不在 P015 V1 採用。若未來採用，優先順序應是：file-level directive > `_conventions.md` project setting > `any` fallback。
...
| 檔案 | 變更類型 | 說明 |
|---|---|---|
| `lib/init.js`（P014 implement 後新增） | 修改 | 在 `npx dflow init` Q&A flow 加入 `prose-language` 問題；將答案寫入 `dflow/specs/shared/_conventions.md` |
| `sdd-ddd-webforms-skill/references/init-project-flow.md` | 修改 | 將 prose-language intake 納入 init flow spec；若 P014 已落地，定位為 CLI internal flow spec |
...
| `sdd-ddd-webforms-skill/references/{new-feature-flow,modify-existing-flow,new-phase-flow,finish-feature-flow}.md` | 修改 / 確認 | 若現有 flow 沒有固定讀 `_conventions.md`，補上讀取 project prose language 的步驟；bug-fix 若由 `modify-existing-flow.md` 承接，也需套用同一規則；若已由 `SKILL.md` 統一涵蓋則只需確認 |
| `sdd-ddd-core-skill/references/{new-feature-flow,modify-existing-flow,new-phase-flow,finish-feature-flow}.md` | 修改 / 確認 | 同 WebForms |
...
採用 (B) 時工作量為中，因為它與 P014 Sub-wave 1 的 CLI init Q&A flow 強耦合，且需要同步雙版 skill / init flow / scaffolding / README。實際修改面約 8-14 檔；若 flow reference 已有統一讀取 `_conventions.md` 的步驟，檔數可下降。

若 P014 延後而先採用 fallback (C)，工作量為小：主要修改雙版 `_conventions.md`、雙版 `SKILL.md`、必要的 workflow 指引與 README，約 5-8 檔。

建議 implement 順序：

1. 先 implement / approve `proposals/PROPOSAL-014-cli-init-and-dflow-root-namespace.md` 的 Sub-wave 1，讓 `npx dflow init` 與 `dflow/specs/shared/_conventions.md` 寫入點成形。
2. 再 implement P015 default (B)，把 `prose-language` 問題接進同一個 init Q&A flow。
3. 若 P014 與 P015 同批 implement，建議在同一批 PR / commit series 中處理 `_conventions.md` 與 init flow，避免同一批 scaffolding 連續改兩次；但 proposal 仍維持分開，因為兩者解決層次不同。
4. 若 P014 未通過或排程延後，先 implement P015 fallback (C)，不等待 CLI。
...
| `proposals/PROPOSAL-014-cli-init-and-dflow-root-namespace.md` | 依賴 / 互補 | 候選 (B) 強依賴 P014 的 `npx dflow init` Q&A flow 與 `dflow/specs/shared/_conventions.md` 目標路徑；候選 (C) 可獨立於 P014 |
