...
4. 依 review `F-07`，`new-feature` / `modify-existing` / `new-phase` / `finish-feature` / `bug-fix` 五條 prose-generating flow **必須**在 Step 1 讀取 `dflow/specs/shared/_conventions.md` 的 `Prose Language` 段。任何 flow 跳過此步即違背 P015 契約。
...
| `sdd-ddd-webforms-skill/references/{new-feature,modify-existing,new-phase,finish-feature,bug-fix}-flow.md` | 修改（mandatory） | 依 review `F-07`，Step 1 必須加讀 `dflow/specs/shared/_conventions.md` 的 prose-language 規則；任何 flow 跳過此步即違背 P015 契約 |
| `sdd-ddd-core-skill/references/{new-feature,modify-existing,new-phase,finish-feature,bug-fix}-flow.md` | 修改（mandatory） | 同 WebForms |
